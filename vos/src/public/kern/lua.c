...
int kernel_lua_namespace_define(lua_State *L) {
    ...
    Namespace *ns = lua_touserdata(L, lua_upvalueindex(2));
    const char *query = lua_tostring(L, 1);
    Process *process = lua_touserdata(L, lua_upvalueindex(3));
    // Function* function =
    ...
    lua_pushvalue(L, lua_upvalueindex(1)); // Assuming the namespace methods table is the third upvalue

    // Return the namespace methods table for chaining
    return 1;
}
...
int kernel_lua_namespace(lua_State *L) {
    ...
    lua_pushvalue(L, -1); // Duplicate the namespace methods table to be used as an upvalue
    ...
    // Now push the closure with the namespace, kernel, and the namespace methods table as upvalues
    lua_pushcclosure(L, kernel_lua_namespace_define, 3);
