...
#else
    static const char *level_strings[6] = {"[FATAL] - ", "[ERROR] - ", "[WARN] - ", "[INFO] - ", "[DEBUG] - ",
            "[TRACE] - "};
#endif
    
    b8 is_error = level < LOG_LEVEL_WARN;
    
    char formatted_message[KILOBYTES(32)];
    memset(formatted_message, 0, sizeof(formatted_message));
    
    va_list arg_ptr;
            va_start(arg_ptr, message);
    vsnprintf(formatted_message, KILOBYTES(4), message, arg_ptr);
            va_end(arg_ptr);
