...
void linear_allocator_create(u64 total_size, void *memory, linear_allocator *out_allocator) {
    if (out_allocator) {
        ...
        if (memory) {
            ...
            out_allocator->memory = vallocate(total_size, MEMORY_TAG_LINEAR_ALLOCATOR);
        }
    }
}
...
void linear_allocator_destroy(linear_allocator *allocator) {
    if (allocator) {
        ...
        if (allocator->owns_memory && allocator->memory) {
            vfree(allocator->memory, allocator->total_size, MEMORY_TAG_LINEAR_ALLOCATOR);
        }
        allocator->memory = 0;
...
void *linear_allocator_allocate(linear_allocator *allocator, u64 size) {
    if (allocator && allocator->memory) {
        if (allocator->allocated + size > allocator->total_size) {
            ...
            verror("linear_allocator_allocate - Tried to allocate %lluB, only %lluB remaining.", size, remaining);
            return 0;
        }
    ...
    verror("linear_allocator_allocate - provided allocator not initialized.");
    return 0;
}
...
void linear_allocator_free_all(linear_allocator *allocator, b8 clear) {
    if (allocator && allocator->memory) {
        allocator->allocated = 0;
        if (clear) {
            vzero_memory(allocator->memory, allocator->total_size);
        }
    }
}