 ...
 */
VAPI b8 queue_create(queue* out_queue, u32 element_size);
/**
 * @brief Destroys the given queue.
 *
 * @param s A pointer to the queue to be destroyed.
 */
VAPI void queue_destroy(queue* s);
 ...
 */
VAPI b8 queue_push(queue* s, void* element_data);
 ...
 * @return True on succcess; otherwise false.
 */
VAPI b8 queue_peek(const queue* s, void* out_element_data);
 ...
 * @return True on succcess; otherwise false.
 */
VAPI b8 queue_pop(queue* s, void* out_element_data);
