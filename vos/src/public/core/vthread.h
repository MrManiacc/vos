 ...
 */
VAPI b8 kthread_create(pfn_thread_start start_function_ptr, void *params, b8 auto_detach, kthread *out_thread);

/**
 * Destroys the given thread.
 */
VAPI void kthread_destroy(kthread *thread);

/**
 * Detaches the thread, automatically releasing resources when work is complete.
 */
VAPI void kthread_detach(kthread *thread);

/**
 * Cancels work on the thread, if possible, and releases resources when possible.
 */
VAPI void kthread_cancel(kthread *thread);
 ...
 */
VAPI b8 kthread_wait(kthread *thread);
