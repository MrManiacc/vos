...
Dict *dict_create(u64 size, hash_function *hash_func) {
    Dict *table = vallocate(sizeof(Dict), MEMORY_TAG_DICT);
    table->size = size;
    table->hash_func = hash_func;
    table->elements = vallocate(sizeof(Entry *) * size, MEMORY_TAG_DICT);
...
void dict_delete(Dict *table) {
    ...
    for (u32 i = 0; i < table->size; i++) {
        ...
        while (e != NULL) {
            Entry *next = e->next;
            vfree(e->key, string_length(e->key) + 1, MEMORY_TAG_STRING);
            vfree(e, sizeof(Entry), MEMORY_TAG_DICT);
            e = next;
        }
    }
    vfree(table->elements, sizeof(Entry *) * table->size, MEMORY_TAG_DICT);
    vfree(table, sizeof(Dict), MEMORY_TAG_DICT);
}
...
b8 dict_set(Dict *table, const char *key, void *value) {
    ...
    Entry *e = vallocate(sizeof(Entry), MEMORY_TAG_DICT);
...
void *dict_remove(Dict *table, const char *key) {
    ...
    vfree(temp->key, string_length(temp->key) + 1, MEMORY_TAG_STRING);
    vfree(temp, sizeof(Entry), MEMORY_TAG_DICT);
    return result;
}
