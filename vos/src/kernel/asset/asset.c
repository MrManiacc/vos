...
AssetHandle *asset_load(AssetPath path) {
    vdebug("Loading asset %s", path.path)
    ...
    for (u32 i = 0; i < asset_context->asset_loader_count; ++i) {
        ...
        if (loader->is_supported(&path)) {
            ...
            dict_set(asset_context->asset_map, path.path, asset);
            vdebug("Asset [%s] loaded at path %s.", loader->name, path.path)
            return asset;
        }
    }
...
b8 asset_unload(AssetPath path) {
    ...
    // Check if the asset is supported.
    for (u32 i = 0; i < darray_length(asset_context->asset_loaders); i++) {
        AssetLoader *loader = darray_get(asset_context->asset_loaders, i);
        if (loader->is_supported(&path)) {
            asset->state = ASSET_STATE_UNLOADING;
            loader->unload(asset);
