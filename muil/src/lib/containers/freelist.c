...
//                       (sizeof(void *) * sizeof(freelist_node)));  // NOTE: This might have a remainder, but that's ok.
    ...
    *memory_requirement = sizeof(internal_state) + (sizeof(freelist_node) * max_entries);
    ...
    vzero_memory(out_list->memory, *memory_requirement);
    ...
    vzero_memory(state->nodes, sizeof(freelist_node) * state->max_entries);
...
void freelist_destroy(freelist *list) {
    if (list && list->memory) {
        ...
        vzero_memory(list->memory, sizeof(internal_state) + sizeof(freelist_node) * state->max_entries);
        list->memory = 0;
    }
}
...
b8 freelist_allocate_block(freelist *list, u64 size, u64 *out_offset) {
    ...
    vwarn("freelist_find_block, no block with enough free space found (requested: %lluB, available: %lluB).", size,
          free_space);
    return false;
}
...
b8 freelist_free_block(freelist *list, u64 size, u64 offset) {
    ...
    vwarn("Unable to find block to be freed. Corruption possible?");
    return false;
}
...
b8 freelist_resize(freelist *list, u64 *memory_requirement, void *new_memory, u64 new_size, void **out_old_memory) {
    ...
    *out_old_memory = list->memory;
    ...
    // The block's layout is head* first, then array of available nodes.
    vzero_memory(list->memory, *memory_requirement);
