...
//            vdebug("Freeing string: %s", allocation->string)
            vfree(allocation->string, allocation->length + 1, allocation->tag); // Free the string
            vfree(allocation, sizeof(StringAllocation), MEMORY_TAG_STRING); // Free the allocation struct
...
char *string_allocate_empty(u64 length) {
    char *copiedString = vallocate(length + 1, MEMORY_TAG_STRING);
    vzero_memory(copiedString, length + 1);
    
    StringAllocation *allocation = vallocate(sizeof(StringAllocation), MEMORY_TAG_STRING);
...
char *string_allocate_sized(const char *input, u64 length) {
    ...
    char *copiedString = vallocate(length + 1, MEMORY_TAG_STRING);
    vcopy_memory(copiedString, input, length + 1);
...
inline char *string_split(const char *str, const char *delimiter) {
    ...
    vfree(copy, string_length(copy) + 1, MEMORY_TAG_STRING);
    return token;
}
...
char *string_concat(const char *str0, const char *str1) {
    ...
    char *result = vallocate(str0_len + str1_len + 1, MEMORY_TAG_STRING); // Allocate once
    vcopy_memory(result, str0, str0_len);
    vcopy_memory(result + str0_len, str1, str1_len + 1); // Include null terminator
...
char *string_to_lower(const char *input) {
    ...
    char *lowercase_str = vallocate(len + 1, MEMORY_TAG_STRING);
    for (u32 i = 0; i < len; ++i) {
...
inline i32 string_split_count(const char *str, const char *delimiter) {
    ...
    vfree(copy, strlen(str) + 1, MEMORY_TAG_STRING); // Replace string_length with the correct function if necessary
    return count;
}
...
char *string_repeat(const char *str, u64 count) {
    if (!str) return null;
    u64 str_len = string_length(str);
    char *result = vallocate(str_len * count + 1, MEMORY_TAG_STRING); // Allocate once
    for (u64 i = 0; i < count; ++i) {
        vcopy_memory(result + i * str_len, str, str_len);
    }
