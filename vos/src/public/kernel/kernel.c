...
// void kernel_watch_process(Proc *process);
// b8 kernel_process_reload(Kernel *kernel, u16 code, void *sender, void *listener_inst, event_context data);
//
//
...
//     event_register(kernel_context, EVENT_CODE_WATCHED_FILE_WRITTEN, 0, kernel_process_reload);
...
//     });
//     dict_set(processes_by_name, process->fs_node->path, process);
...
//     if (should_watch) kernel_watch_process(process);
//     return process;
// }
//
// void kernel_watch_process(Proc *process) {
//     FsNode *script_node = process->fs_node;
//     if (script_node == null) {
//         vwarn("Attempted to watch process with null script node")
//         return;
//     }
//     char *absolute_path = path_absolute(script_node->path);
//     if (absolute_path == null) {
//         vwarn("Failed to get absolute path for process %s", script_node->path)
//         return;
//     }
//     char *platform_path = path_to_platform(absolute_path);
//     script_node->watch_id = platform_watch_file(platform_path);
//     vinfo("Watching process %s, watch, %d", script_node->path, *script_node->watch_id)
// }
//
// b8 kernel_process_reload(Kernel *kernel, u16 code, void *sender, void *listener_inst, event_context data) {
//     if (!kernel_initialized) {
//         vwarn("Attempted to reload process before kernel initialization")
//         return false;
//     }
//     if (code != EVENT_CODE_WATCHED_FILE_WRITTEN) {
//         vwarn("Invalid event code %d", code)
//         return false;
//     }
//     u32 watch_id = data.data.u32[0];
//     Proc *process = null;
//     for (u32 i = 0; i < MAX_PROCESSES; i++) {
//         if (kernel->processes[i] != null) {
//             if (*kernel->processes[i]->fs_node->watch_id == watch_id) {
//                 process = kernel->processes[i];
//                 break;
//             }
//         }
//     }
//     if (process == null) {
//         vwarn("Failed to find process with watch id %d", watch_id)
//         return false;
//     }
//     vinfo("Reloading process %s", process->process_name)
//     lua_State *L = process->lua_state;
//     if (L == null) {
//         vwarn("Failed to reload process %s; lua state is null", process->process_name)
//         return false;
//     }
//     //delete the process from the processes_by_name dict
//     ProcessState state = process->state;
//     b8 should_start = false;
//     if (state == PROCESS_STATE_RUNNING) {
//         should_start = true;
//     }
//     process->state = PROCESS_STATE_STOPPED;
//     dict_remove(processes_by_name, process->fs_node->path);
//     FsNode *node = vfs_reload_node(kernel->fs_context, process->fs_node->path);
//     process = kernel_create_process(node, false);
//     if (should_start) {
