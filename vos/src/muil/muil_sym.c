...
static b8 validateType(TypeCheckingVisitor *self, Type *type);
...
static void validateTypes(TypeCheckingVisitor *self) {
...
static Type *evalExpressionType(TypeCheckingVisitor *self, ASTNode *node) {
...
static void visitAssignment(TypeCheckingVisitor *self, AssignmentNode *node) {
...
static void visitPropertyNode(TypeCheckingVisitor *self, PropertyNode *node) {
    //The type is semi-ambiguous. We may have to infer the type from the expression
    if (node->type == null) {
//        Type *type = evalExpressionType(self, node->value);
//        if (type == null) {
...
static void register_primitive_types(TypeCheckingVisitor *self) {
...
static void destroy_primitive_types(TypeCheckingVisitor *self) {
...
void visit_program_ast(TypeCheckingVisitor *self, ProgramAST *program) {
...
VAPI TypeCheckingVisitor *typechecker_new() {
    ...
    visitor->base.visitPropertyNode = (void (*)(IRVisitor *, PropertyNode *)) visitPropertyNode;
    visitor->base.visitProgramAST = (void (*)(IRVisitor *, ProgramAST *)) visit_program_ast;
    visitor->base.visitAssignmentNode = (void (*)(IRVisitor *, AssignmentNode *)) visitAssignment;
